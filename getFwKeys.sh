...
if [[ ! $(echo $wikiPage | grep 'Keys') ]]; then echo "\niOS details not found! Please check the information provided!" ; sleep 2; echo "Exiting..."; sleep 2; exit ; fi
...
if [[ $jsonKeysLink == "https://theapplewiki.com" ]]; then echo "\nKeys not availablefor this iOS...try a different one!" ; sleep 2; echo "Exiting..."; sleep 2; exit ; fi

read -p "Download json file?(y/n): " downloadJson
if [[ $downloadJson == [y,Y] ]]; then curl -sL -o "$deviceId"_"$ipswbuildid"_"$iosVersion.json" $jsonKeysLink; fi
echo "\nFile saved as: $deviceId"_"$ipswbuildid"_"$iosVersion.json\n"
